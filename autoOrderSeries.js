...
function checkIfVideos(data) {
	var dirFiles = fs.readdirSync(pathTorrents + data.folder), video = '', extraFoldertoRemove = null;
logger("Recorro el directorio");

	// Si solo hay un fichero y es un directorio, entro a él
logger("----> A ver si hay una carpeta extra");
	if (dirFiles.length === 1 && fs.lstatSync(pathTorrents+data.folder+'\\'+dirFiles[0]).isDirectory()) {
		// directorio original a borrar
		extraFoldertoRemove = pathTorrents + data.folder
		// Añado la carpeta a la ruta
		data.folder += '\\' + dirFiles[0] + '\\';
logger("----> La hay. Nuevo directorio: " + data.folder);
		// Actualizo el directorio de ficheros
		dirFiles = fs.readdirSync(pathTorrents + data.folder);
	}
	...
	if (video === '') {
		...
		return {
			...
			extraFoldertoRemove: extraFoldertoRemove
		};
	}
}
        ...
        if (existe) {
            ...
            if (moved) {
                ...
                try {
					...
					// Si habia una carpeta extra la borro también
					if (fichero.extraFoldertoRemove !== null) {
	                    fs.unlinkSync(fichero.extraFoldertoRemove);
					}
